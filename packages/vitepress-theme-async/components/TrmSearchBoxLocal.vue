...
onKeyStroke("ArrowDown", () => {
	checkIndex.value = Math.min(checkIndex.value + 1, results.value.length - 1)
})

onKeyStroke("ArrowUp", () => {
	checkIndex.value = Math.max(checkIndex.value - 1, 0)
})

onKeyStroke("Enter", () => {
	const result = results.value[checkIndex.value]
	if (result) document.getElementById(`result${checkIndex.value}`)?.click()
})
</script>
<template>
	<Teleport to="body">
		<div ref="el" class="trm-search-popup">
			...
			<div class="trm-search-wrapper">
				<div class="form trm-search-form">
					...
					<input v-model="filterText" ref="searchInput" id="localsearch-input" aria-labelledby="localsearch-label" class="trm-search-input" autofocus type="text" placeholder="内容搜索..." />
				...
				<ul ref="resultsEl" class="trm-search-result-container">
					<li v-for="p, i in results" :key="p.id" role="option" @mouseenter="() => onmouseenter(i)">
						<a :id="`result${i}`" class="trm-result" :class="{ checked: i === checkIndex }" :href="p.id" @click="$emit('close')">
							<div class="titles">
						...
						<li>
							<kbd class="command-palette-commands-key">
								<svg width="15" height="15" viewBox="0 0 24 24">
									<path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19V5m-7 7l7-7l7 7"></path>
								</svg>
							</kbd>
							<kbd class="command-palette-commands-key">
								<svg width="15" height="15" viewBox="0 0 24 24">
									<path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v14m7-7l-7 7l-7-7"></path>
								</svg>
							</kbd>
